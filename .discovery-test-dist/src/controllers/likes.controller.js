...
const likesService = __importStar(require("../services/likes.service"));
const likes_validator_1 = require("../validators/likes.validator");
async function likeStory(request, response) {
    const storyId = likes_validator_1.storyIdSchema.parse(request.params.id);
    const result = await likesService.likeStory(request.user.id, storyId);
    response.status(201).json({ data: result });
}
async function unlikeStory(request, response) {
    const storyId = likes_validator_1.storyIdSchema.parse(request.params.id);
