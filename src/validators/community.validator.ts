...
export const communityQuery = z.object({
  tab: z
...
export const communityPostInput = z
  .object({
    content: text(4000),
    ...
    story_id: communityId
      .nullable()
      .optional(),

    chapter_id: communityId
      .nullable()
      .optional(),
  ...
  .superRefine((value, ctx) => {
    ...
    if (
      ...
      (
        !value.poll_options ||
        new Set(
          value.poll_options.map((option) => option.toLowerCase())
        ).size !== value.poll_options.length
      )
    ) {
      ctx.addIssue({
        code: 'custom',
...
export const communityCommentInput = z.object({
  ...
  parent_comment_id: communityId
    .nullable()
    .optional(),
});

export const communityEditInput = z.object({
  content: text(4000),
});
