...
export async function uploadCover(request: any, response: any) {
	const url = await uploadsService.uploadImage('cover', request.user.id, request.file);
	response.status(201).json({ data: { url } });
}

export async function uploadAvatar(request: any, response: any) {
	const url = await uploadsService.uploadImage('avatar', request.user.id, request.file);
	response.status(201).json({ data: { url } });
}
